...
#             self.vis=Render.CLAHEbrightness(self.vis,0,tileGridSize=(20,20),clipLimit=2)
    ...
    def scaleAndLogTracks(self, i):
        ...
        norm_velocity = np.sqrt(np.sum(self.V ** 2, axis=1))
        avg_velocity = np.mean(norm_velocity)
        avg_velocity_x = np.mean(np.abs(self.V[:, 0]))  # Average velocity in x-axis
        avg_velocity_y = np.mean(np.abs(self.V[:, 1]))  # Average velocity in y-axis
        ...
        if len(self.V) == 0:
            ...
            print('Displacement maximum = ' + str(np.max(np.absolute(self.V))) +  ' ' + self.unit[0] + '/' + self.unit[1])

            print(f'Average norm velocity: {avg_velocity:.2f} {self.unit[0]}/{self.unit[1]}')

            print(f'Average velocity on y-axis = {avg_velocity_y:.2f} {self.unit[0]}/{self.unit[1]}')

            print(f'Average velocity on x-axis = {avg_velocity_x:.2f} {self.unit[0]}/{self.unit[1]}')
    ...
    def scaleAndLogFlow(self, i):
        ...
        if self.scaled:    
            self.avg_velocity_list.append({
            'step': self.incr,
            'norm_velocity': avg_velocity,
            'velocity_x': avg_velocity_x,
            'velocity_y': avg_velocity_y
        })

    def writeVelocityEachStep(self, csv_filename=None):
    # Define the filename for the CSV
        if csv_filename is None:
            csv_filename = 'velocity.csv'

        # Calculate averages for each velocity component
        if len(self.avg_velocity_list) > 0:
            avg_norm_velocity = sum(d['norm_velocity'] for d in self.avg_velocity_list) / len(self.avg_velocity_list)
            avg_velocity_x = sum(d['velocity_x'] for d in self.avg_velocity_list) / len(self.avg_velocity_list)
            avg_velocity_y = sum(d['velocity_y'] for d in self.avg_velocity_list) / len(self.avg_velocity_list)
        else:
            avg_norm_velocity = avg_velocity_x = avg_velocity_y = 0.0  # Handle empty list case
            
        # Open the file in write mode
        with open(csv_filename, mode='w', newline='') as file:
            writer = csv.writer(file)
    
            # Write the header with columns for step, norm_velocity, velocity_x, and velocity_y
            writer.writerow(['Step', 'Norm Velocity', 'Velocity X', 'Velocity Y'])
    
            # Write the data from self.avg_velocity_list
            for step, velocity_data in enumerate(self.avg_velocity_list, start=1):
                writer.writerow([
                    step,  # The step index
                    velocity_data['norm_velocity'],  # Norm of velocity
                    velocity_data['velocity_x'],     # Velocity in x-axis
                    velocity_data['velocity_y']      # Velocity in y-axis
                ])

            writer.writerow([
                'Mean',  # Label for the last row
                avg_norm_velocity,  # Average of norm velocity
                avg_velocity_x,     # Average of velocity in x-axis
                avg_velocity_y      # Average of velocity in y-axis
            ])
    
        print(f"Average velocity data saved to {csv_filename}")

    def plot_average_velocity(self, type='norm'):
        time = np.arange(1, len(self.avg_velocity_list) + 1) * (self.paramVecTime['step'] / self.fps)
        
        # Extract velocities
        velocities = np.array([[v['step'], v['norm_velocity'], v['velocity_x'], v['velocity_y']] for v in self.avg_velocity_list])
    
        # Plot based on the provided velocity_type
        if type == 'norm':
            plt.figure(figsize=(8, 5))
            plt.plot(time, velocities[:, 1], 'b.-', label='Norm Velocity')
            plt.xlabel('Time (seconds)')
            plt.ylabel(f'Average Norm Velocity ({self.unit[0]}/{self.unit[1]})')
            plt.grid(True)
            plt.show()
            
            for step, norm_velocity in velocities[:, [0, 1]]:
                print(f"{step}, {norm_velocity}")
    
        elif type == 'velocity_x':
            plt.figure(figsize=(8, 5))
            plt.plot(time, velocities[:, 2], 'r.-', label='X-axis Velocity')
            plt.xlabel('Time (seconds)')
            plt.ylabel(f'Average X-axis Velocity ({self.unit[0]}/{self.unit[1]})')
            plt.grid(True)
            plt.show()
            
            for step, norm_velocity in velocities[:, [0, 2]]:
                print(f"{step}, {norm_velocity}")
    
        elif type == 'velocity_y':
            plt.figure(figsize=(8, 5))
            plt.plot(time, velocities[:, 3], 'g.-', label='Y-axis Velocity')
            plt.xlabel('Time (seconds)')
            plt.ylabel(f'Average Y-axis Velocity ({self.unit[0]}/{self.unit[1]})')
            plt.grid(True)
            plt.show()
            
            for step, norm_velocity in velocities[:, [0, 3]]:
                print(f"{step}, {norm_velocity}")
    
        else:
            print(f"Invalid velocity type: {velocity_type}. Choose 'norm', 'velocity_x', or 'velocity_y'.")
    ...
    def writeVelocityField(
        ...
        if self.fieldResults == 'time-serie':
            if filename is None:
                self.filename = 'frame' + str(self.vec[0]) + 'to' + str(self.vec[-1]) + '_step' + str(
                    self.paramVecTime['step']) + '_shift' + str(self.paramVecTime['shift']) + '_' + self.unit[0] + self.unit[1]
            ...
            if filename is None:
                self.filename = 'velocityfeaturesaccumulation_frame_' + str(self.vec[0]) + 'to' + str(self.vec[-1]) + '_step' + str(
                    self.paramVecTime['step']) + '_shift' + str(self.paramVecTime['shift'])+ '_' + self.unit[0] + self.unit[1]
            self.variables = [['Ux_[' + self.unit[0] + '.' + self.unit[1] +
                               '-1]', self.UxTot], ['Uy_[' +  self.unit[0] + '.' + self.unit[1] + '-1]', self.UyTot]]
            if fileFormat == 'hdf5':
