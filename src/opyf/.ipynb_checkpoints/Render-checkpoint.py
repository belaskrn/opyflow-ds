...
def opyfPlot(grid_x, grid_y, gridVx, gridVy, Xdata, Vdata, setPlot, vis=None, Ptype='norm', namefig='Opyf', vlim=None, scale=None, cmap='jet', alpha=0.6, width=0.002, nvec=3000, respx=32, ROIvis=None, **args):
...
#        self.setExtent(self.paramPlot['extentFrame'])
    ...
    def plot(self, Field=None,
             ...
             namefig='Opyf', scale=None, cmap='jet',
...
def opyfQuiverPointCloudColored(X, V, fig=None, ax=None, nvec=3000, normalize=False, **args):
    ...
    from matplotlib.colors import Normalize
    if 'cmap' not in args:
        args['cmap'] = mpl.cm.jet
    cmap = args.get('cmap', mpl.cm.jet)
    del args['cmap']
    fig, ax = getax(fig=fig, ax=ax)
    # one over N
    # Select randomly N vectors
...
def opyfPointCloudColoredScatter(X, V, fig=None, ax=None, cmapCS=mpl.cm.jet, **args):
    from matplotlib.colors import Normalize
    fig, ax = getax(fig=fig, ax=ax)
    if 'cmap' in args:
        del args['cmap']
...
def opyfPlotRectilinear(vecX, vecY, gridVx, gridVy, setPlot, Xdata=None, Vdata=None, vis=None, Hfig=9, Ptype='norm', namefig='Opyf', vlim=None, scale=None, cmap='jet', alpha=0.6, width=0.002, nvec=3000, res=32, ROIvis=None, **args):
